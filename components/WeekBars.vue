<template>
  <div>
    ...
    <p v-if="rows.length === 0" class="mt-4 italic text-mute">No entries in this range.</p>
    ...
    <ul v-else class="mt-2" role="list" aria-label="Time by tag">
      <li v-for="row in rows" :key="row.label" class="bar-row">
        ...
        <span class="tnum text-sm">{{ fmtDuration(row.count) }}</span>
      </li>
    </ul>
    <p class="form-hint mt-2">Each entry counts once, under its first tag. Fifteen minutes each.</p>
  </div>
</template>
